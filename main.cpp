...
// 这里是你通过编译 RouteData.proto 生成的头文件
...
static std::vector<PlanLineData> parseWaylinesWPML(const std::string& xmlPath)
{
...
int main(int argc, char** argv)
{
    ...
    // 调用解析函数
    std::vector<PlanLineData> lines = parseWaylinesWPML(argv[1]);
    std::cout << "Found " << lines.size() << " Folder(s) in " << argv[1] << "\n";
